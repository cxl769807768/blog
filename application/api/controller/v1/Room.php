...
class Room extends Common
{
    ...
    public function index()
    {
        if (strtolower($this->request->method()) == 'get') {
            ...
            $res = [];
            $res["total"] = model('Room')->where($where)->count();
            $res["list"] = $lists;
            if (empty($res)) {
    ...
    public function save(Request $request)
    {
        if (strtolower($this->request->method()) == 'post') {
            ...
            foreach ($data['slideshow'] as $key => $value) {
                ...
                $temp[$key]['mod'] = 'room_slideshow';
            }
            $data['slideshow'] = serialize(array_column($temp,'url'));
            $res = model('room')->allowField(true)->save($data);
            if ($res) {
                $insert_cover = model('ImgManage') -> save(['mod' => 'room_cover', 'url' => $data['cover']]);
    ...
    public function update(Request $request, $id)
    {
        if (strtolower($this->request->method()) == 'post') {
            ...
            $old = model('room')->where('id',$id)->find()->toArray();
            $data['cover'] = str_replace($this->request->domain(), '', $data['cover']);
            ...
            $data['slideshow'] = serialize(array_column($temp,'url'));
            $res = model('room')->isUpdate(true)->allowField(true)->save($data);
            if ($res) {
                if($old['cover']!==$data['cover']){
                    model('ImgManage')->where(['mod'=>'room_cover', 'url'=>$old['cover']])->delete();
                    $insert_cover = model('ImgManage') -> save(['mod' => 'room_cover', 'url' => $data['cover']]);
                    if (empty($insert_cover)) {
                        $this -> return_msg(0, '添加封面信息失败');
                    }
                ...
                if($old['slideshow']!==$data['slideshow']){
                    ...
                    foreach ($oldSlideshow as $key => $val){
                        model('ImgManage')->where(['mod'=>'room_slideshow', 'url'=>$val])->delete();
                    }
                    if (empty($insert_slideshow)) {
                        $this -> return_msg(0, '添加图片管理轮播信息失败');
                    }
                    ...
                    if (!empty($shu_ress)) {
                        ...
                        if (!empty($diff)) {
                            ...
                            foreach ($shu_ress[1] as $key => $value) {
                                model('ImgManage')->where(['mod'=>'room_introduce', 'url'=>str_replace($this->request->domain(), '', $value)])->delete();
                            }
                            $intro = [];
                            foreach ($ress[1] as $key => $value) {  // 新增传递的信息
                                $intro[$key]['url'] = str_replace($this->request->domain(), '', $value);
                                $intro[$key]['mod'] = 'room_introduce';
                            }
    ...
    public function delete($id)
    {
        if (strtolower($this->request->method()) == 'post') {
            ...
            $old = model('room')->where('id', $id)->find()->toArray();
            $res = model('room')->where('id', $id)->delete();
            if($res){
