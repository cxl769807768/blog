...
class Redis
{
     ...
     * @param $key
     * @param $key1
     * @return string   弹出列表最后一个值，同时插入到另一个列表头部，并返回该值(弹出$key最先加入的消息，同时放入$key1监听队列)
     */
    public function rpoplpush($key,$key1)
    {
        return $this->redis->rpoplpush($key,$key1);
    }
    /**
     * 删除值为vaule的count个元素
     ...
     * @param $key
     * @param $value
     * @return bool  判断 $value 元素是否是集合 key 的成员
     */
